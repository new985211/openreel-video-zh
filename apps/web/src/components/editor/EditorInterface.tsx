    ...
    const initAll = async () => {
      try {
        ...
        if (!currentState.initialized && !currentState.initializing) {
          setInitStatus(t('editor.initVideoEngine'));
          await initialize();
        ...
        if (!engineState.initialized) {
          throw new Error(
            engineState.initError || t('editor.engineInitFailed'),
          );
        }

        setInitStatus(t('editor.initMediaBridge'));
        await initializeMediaBridge();
        if (!isMounted) return;

        setInitStatus(t('editor.initPlaybackBridge'));
        await initializePlaybackBridge();
        if (!isMounted) return;

        setInitStatus(t('editor.initRenderBridge'));
        await initializeRenderBridge();
        if (!isMounted) return;

        setInitStatus(t('editor.initEffectsBridge'));
        ...
        setInitStatus(t('editor.initTransitionBridge'));
        try {
    ...
    return (
      <div className="w-full h-full bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-text-secondary text-sm">{t('editor.initializing')}</p>
