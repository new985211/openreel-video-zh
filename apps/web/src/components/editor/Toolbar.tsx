...
export const Toolbar: React.FC = () => {
  ...
  const handleRecordingComplete = useCallback(
    async (screenBlob: Blob, webcamBlob?: Blob) => {
      if (!screenBlob || screenBlob.size === 0) {
        toast.error(
          t('toolbar.recordingFailed'),
          t('toolbar.noVideoCaptured'),
        );
        return;
      }
      ...
      if (screenResult.success) {
        ...
        errors.push(
          screenResult.error?.message || t('toolbar.failedToImportScreen'),
        );
      }
      ...
      if (webcamBlob && webcamBlob.size > 0) {
        ...
        if (webcamResult.success) {
          ...
          errors.push(
            webcamResult.error?.message || t('toolbar.failedToImportWebcam'),
          );
        }
      }
      ...
      if (importCount > 0) {
        toast.success(
          t('toolbar.recordingImported', { count: importCount }),
          webcamBlob && webcamBlob.size > 0
            ? t('toolbar.screenAndWebcamAdded')
            : t('toolbar.screenRecordingAdded'),
        );
  ...
  const exportOptions: Array<{
    ...
    {
      label: t('toolbar.exportOptions.mp4Standard'),
      icon: Zap,
    ...
    ...(isVertical
      ...
      : [
          {
            label: t('toolbar.exportOptions.4kStandard'),
            icon: FileVideo,
    ...
    {
      label: t('toolbar.exportOptions.1080pHighQuality'),
      icon: FileVideo,
      desc: "1920×1080 30fps - High bitrate",
      type: "1080p-high",
    },
    {
      label: t('toolbar.exportOptions.1080p60fps'),
      icon: FileVideo,
      desc: "1920×1080 - Smooth playback",
      type: "1080p-60",
    },
    {
      label: t('toolbar.exportOptions.audioOnlyWav'),
      icon: Music,
          ...
          {exportState.isExporting ? (
            ...
            <DropdownMenu open={isExportOpen} onOpenChange={setIsExportOpen}>
              <DropdownMenuTrigger asChild>
                ...
                >
                  <span className="text-sm tracking-wider">{t('toolbar.exportButton')}</span>
                  <ChevronDown
                    size={14}
