    ...
    if (route === "new") {
      ...
      if (params.preset) {
        ...
        if (preset) {
          ...
          projectName = `${t('welcome.newProject')} ${presetKey.charAt(0).toUpperCase() + presetKey.slice(1).replace(/-/g, " ")}`;
        }
    ...
    <TooltipProvider>
      <div className="h-screen w-screen bg-background text-text-primary overflow-hidden">
        ...
        {isSharePage ? (
          ...
          <Suspense fallback={<LoadingSpinner message={t('toolbar.loadingEditor')} />}>
            <EditorInterface />
          </Suspense>
        )}
        <ToastContainer />
